...
//                    << ",";
            ...
            //out << "label      :";
            for( int i = 0; i < N; i++ )//遍历所有的点，与哪个中心点距离最小就归到哪个中心中去
            {
                /* center_dist[k]某个聚类中心的颜色总距离         *
                * counters[k]为第k个聚类中点的个数*/
                sample = data.ptr<float>(i);
...
void k_best(float (*Disdance)[300],int *center_result,int *index,int mem[][300],int *mem_num,
    ...
    if(layer == new_nClusters-1){
            index[layer] = index[layer-1]+1;
            for(;index[layer] < nClusters-new_nClusters+layer+2;index[layer]++){
                        int tmp_mem[new_nClusters][300];
                        int tmp_mem_num[new_nClusters];
                        for(int m = 0;m < new_nClusters;m++){
                            for(int t =0;t < nClusters;t++){
                                tmp_mem[m][t] = 0;
                            }
                            tmp_mem_num[m] = 0;
                        }
                        float tmp_sum = distribute(Disdance,tmp_mem,tmp_mem_num,
                                                   index,new_nClusters,nClusters,num,out);
                        //out << "tmp_dis:" << tmp_sum << endl;
                        //初次初始化sum，如果第一次，则index[new_nClusters]和new_nClusters相等
                        if(index[new_nClusters-1] == new_nClusters){
                            //out << "tttttt: " << tmp_sum << endl;
                            sum = tmp_sum;
                            for(int t = 0;t < new_nClusters;t++){
                                mem_num[t] = tmp_mem_num[t];
                                for(int m =0;m < mem_num[t];m++){
                                    mem[t][m] = tmp_mem[t][m];
                                }
                                center_result[t] =index[t];
                            }
                        }
                        else if(tmp_sum < sum){
                            out << "tmp_dis:" << tmp_sum << " " << index[0] -1
                                << " " << index[1] -1 << " " << index[2] -1 <<endl;
                            sum = tmp_sum;
                            for(int t = 0;t < new_nClusters;t++){
                                mem_num[t] = tmp_mem_num[t];
                                for(int m =0;m < mem_num[t];m++){
            ...
            if(layer > 0){
                    index[layer] = index[layer-1]+1;
                    for(;index[layer] < nClusters-new_nClusters+layer+2;index[layer]++){
                        k_best(Disdance,center_result,index,mem,mem_num,
                                    nClusters,new_nClusters,sum,num,layer+1,out);
                    }
            }
            else{
                    for(index[layer] = 1;index[layer] < nClusters-new_nClusters+layer+2;index[layer]++){
                        //out << "layer: " << index[layer] << endl;
                        k_best(Disdance,center_result,index,mem,mem_num,
                                    nClusters,new_nClusters,sum,num,layer+1,out);
                    }
            }
...
void initialization(float Disdance[][300],user_color* color,int (*mem)[300],
                    int *mem_num,int nClusters,int new_nClusters,std::ofstream &out){
    float a[3],b[3];
        ...
        for(int i =0;i < nClusters;i++){
            ...
            a[2] = color[i].b;
                for(int j =0;j < nClusters;j++){
                    ...
                    b[1] = color[j].g;
                    b[2] = color[j].b;
                    if(Disdance[i][j] < 0&& i != j){
//                        out << "Lab:" << a[0] << " " << a[1] << " " << a[2]
//                            << " " << b[0] << " " << b[1] << " " << b[2] << endl;
                        Disdance[i][j] = new_distance(a,b,1);
                        Disdance[j][i] = Disdance[i][j];
                    }
                }
        }
...
float distribute(float Disdance[][300],int mem[][300],int *mem_num,
    ...
    float tmp_sum = 0,tmp_distance;
    for(int i = 0;i < nClusters;i++){
        bool flag = false,is_center = false;
        for(int t = 0;t < new_nClusters;t++){
            if((index[t] - 1) == i){
                is_center = true;
                break;
        ...
        if(!is_center){
        //out << i << endl;
            for(int j = 0;j < new_nClusters;j++){
                if(!flag){
                    tmp_distance = Disdance[i][index[j]-1]*num[i];
                    //out << "f_d: " << num[i] << endl;
                    mem[j][mem_num[j]++] = i;
                    flag = true;
                }else{
                    if(tmp_distance > Disdance[i][index[j]-1]*num[i]){
                        tmp_distance = Disdance[i][index[j]-1]*num[i];
                        //out << "o_d: " << num[i] << endl;
                        mem[j][mem_num[j]++] = i;
                    }
        ...
        tmp_sum += tmp_distance;
        }
    }
    //out << "fsm: " << tmp_sum << endl;
    return tmp_sum;
}
